...
class LangManager_jp extends LangManager
{
    constructor()
    {
      super(LangId.jp);
      this.seg = new JPTinySeg();
    }

    tokenizeText(text)
    {
      console.log("Tokenizing... " + text);
      let [textChunks, chunksAreMatchs] = chunkTextByRegex(text, jpRe);
      let tokens = [];
      for (let i = 0; i < textChunks.length; i++)
      {
        // if this token is not jp, pass the whole thing as not target lang
        ...
        {
          let segs = this.seg.segment(textChunks[i]);
          for (let k of segs)
          {
            console.log("SEG: " + k);
            tokens.push(new LangToken(k, LangId.jp));
          }
        }
    ...
    textContainsTargetLang(text)
    {
        return text != null && jpRe.test(text);
    }

    getBasicTokenInfo(langToken)
    {
      if (langToken.langId != LangId.jp)
      {
        console.log('LangManager_jp.getBasicTokenInfo: tried to get token info for token of different lang id: (' + langToken.token + ", " + langToken.langId + ")");
        return null;
      }

      console.log('LangManager_jp.getBasicTokenInfo:' + 'LangManager ' + LangId.jp + ' getting info for ' + langToken.token);
      let info = db.readTokenInfo(langToken.token, LangId.jp);
      if (info)
        return info;

      console.log('LangManager_jp.getBasicTokenInfo:' +'db has no entry for ' + langToken.token + ', adding new entry');
      info = {
        [MEMORY_STATUS]: 0,
        'notes': '',
      };

      db.writeTokenInfo(langToken.token, LangId.jp, info);
      return info;
    }
    ...
    createTooltipHTML(langToken)
    {
      ...
      const link = `https://jisho.org/search/${langToken.token}`;
